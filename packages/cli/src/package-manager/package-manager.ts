...
export class PackageManager {
	...
	addDep(dep: DetailedDependency, plugin: PluginWrapper, source: DepSource): void {
		...
		if (dep.noalias === true) {
			...
			const depFootPrint = `${dep.packageName}${dep.version}${plugin.pluginImport.path}`;
			const hash = CryptoJS.MD5(depFootPrint)
			const base64Hash = hash.toString();

			alias = `${dep.packageName}-${base64Hash}`
		}
		...
		logger.verbose('Registered dependency', { p: registered.packageName, v: registered.version })
	...
	findPackage(packageName: string, plugin: PluginWrapper): Dep {
		const npmPackage = this._dependencies.find(dep => {
			return dep.packageName === packageName && dep.plugin === plugin;
		})
